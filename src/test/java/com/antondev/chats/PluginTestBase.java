...
public abstract class PluginTestBase {
    ...
    protected void config(Consumer<YamlConfiguration> edit) throws Exception {
        ...
        edit.accept(yaml);
        yaml.save(file);
        assertTrue(plugin.reloadPlugin(), "Modified test config.yml should load");
    }

    protected void data(Consumer<YamlConfiguration> edit) throws Exception {
        var file = plugin.getDataFolder().toPath().resolve("data.yml").toFile();
        var yaml = YamlConfiguration.loadConfiguration(file);
        edit.accept(yaml);
        yaml.save(file);
        assertTrue(plugin.reloadPlugin(), "Modified test data.yml should load");
    }
    ...
    protected static String plain(Component component) { return component == null ? null : PlainTextComponentSerializer.plainText().serialize(component); }
