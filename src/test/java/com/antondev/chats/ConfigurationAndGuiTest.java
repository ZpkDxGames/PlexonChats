...
class ConfigurationAndGuiTest extends PluginTestBase {
    ...
    @Test void completeTwoPointZeroConfigurationMigratesWithoutLosingAnyValue() throws Exception {
        String source;
        try (var fixture = getClass().getResourceAsStream("/fixtures/config-2.0.yml")) {
            assertNotNull(fixture);
            source = new String(fixture.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
        }
        var original = new YamlConfiguration();
        original.loadFromString(source);
    ...
    @Test void releaseMetadataMatchesThreePointZero() {
        assertEquals("3.0", plugin.getPluginMeta().getVersion());
