...
2. Replace the old JAR with `PlexonChats-1.1.0.jar` in `plugins/`. Keep only one PlexonChats JAR.
3. Start the server. Existing 1.0 settings are retained, missing options are added, and the original config is backed up before migration.
...
**Upgrade note:** 1.0 ignored some format settings. Version 1.1.0 actually renders your saved channel/item formats, so the appearance may change. See [the upgrade guide](docs/UPGRADING.md), including rollback and staging checks.
...
See the [configuration guide](docs/CONFIGURATION.md) for placeholders, custom GUI buttons, scheduled-message examples, and DiscordSRV setup. The [bundled config](src/main/resources/config.yml) contains the complete defaults and inline comments.
...
The deployable JAR is `target/PlexonChats-1.1.0.jar`. Test dependencies are not bundled. GitHub Actions runs the same command and uploads the JAR.

Tests cover configuration migration/rejection, safe formatting, routing/permissions, GUI actions, preference persistence, message rotation/delivery, and DiscordSRV API behavior with mocked endpoints. They do **not** log a bot into Discord or replace a staging test on a real Paper server.
